...
def check_objective_value(arg, transforms):
    ...
    if (iteration + 1)%10 == 0 :
        print("Spherical K means at iter %d :  mean cosine similarity %.4f" % (iteration+1, objective))
...
def step(X, centroids, prev_objective : float):
    ...
    objective = jnp.take_along_axis(cos_sim, jnp.expand_dims(assignments, axis=-1), axis=-1).mean()
...
class SphericalKMeans():
    ...
    def _main_loop(self, X, centroids):
        ...
        def while_step(arg):
            ...
            new_centroids, assignments, objective, stop_criteria = step(X, centroids, prev_objective)
        ...
        iteration, centroids, assignments, objective, stop_criteria = jax.lax.while_loop(
            cond,
            while_step,
            (0, centroids, assignments, jnp.inf, jnp.inf)
        )
