...
class KFD:
    ...
    def train(self, Y: np.ndarray, X: np.ndarray, eps: float = 1e-9) -> None:
        ...
        for j in range(nlabels):
            ...
            class_means = np.mean(class_kernel_matrices, axis=-1).reshape(
                -1, 1
            )  # (N, 1)
            centering_matrices = (
                np.eye(label_counts[j]) - 1 / label_counts[j]
            )  # (n_j, n_j)
    ...
    def compute_projected_means(self, Y: np.ndarray, K: np.ndarray) -> None:
        ...
        for l in range(nlabels):
            ...
            self.projected_mean_classes[l] = np.mean(
                self.weights.T @ K[:, class_mask], axis=-1
            )
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        projections = np.transpose(
            self.weights.T @ self.kernel(self.training_data, X)
        )  # (n_test_samples,)

        # choose the class whose the mean projections belonging to this class is the closest to the projected point
        class_ind = np.argmin(
            np.abs(projections[:, np.newaxis] - self.projected_mean_classes), axis=-1
        )
